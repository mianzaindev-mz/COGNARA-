...
export default function CertificatesPage() {
  ...
  const filtered = DEMO_CERTS.filter(
    (c) =>
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {filtered.map((cert) => (
            <div
              key={cert.id}
              className="group relative overflow-hidden rounded-2xl border border-cn-border bg-cn-surface p-6 shadow-[var(--cn-shadow-card)] transition hover:border-cn-orange/30"
            >
              {/* Gold ribbon */}
              ...
              <div className="mb-4 flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-cn-ink">{cert.course}</h3>
                  <p className="text-xs text-cn-ink-muted">by {cert.coach}</p>
                </div>
              </div>
              ...
              <div className="flex gap-2">
                ...
                <button
                  type="button"
                  onClick={() => navigator.clipboard?.writeText(`${window.location.origin}/verify-certificate/${cert.code}`)}
                  className="rounded-xl border border-cn-border px-3 py-2 text-xs font-bold text-cn-ink-muted transition hover:bg-cn-border/30 hover:text-cn-ink"
      ...
      <div className="rounded-2xl border border-cn-border bg-cn-surface/50 p-5">
        ...
        <div className="grid gap-3 text-sm text-cn-ink-muted sm:grid-cols-3">
          <div className="flex gap-2">
            ...
            Each certificate has a unique code like <code className="rounded bg-cn-border/50 px-1 font-mono text-xs">CERT-PY-2026-A1B2C3</code>
          </div>
          <div className="flex gap-2">
            <span className="text-cn-orange">2.</span>
            Anyone can verify at <code className="rounded bg-cn-border/50 px-1 font-mono text-xs">cognara.app/verify-certificate/CODE</code>
          </div>
          <div className="flex gap-2">
