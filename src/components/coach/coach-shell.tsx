...
const nav = [
  ...
  { href: "/coach/library",   label: "My library",  icon: IconLibrary },
      ...
      <>
          ...
          {nav.map(({ href, label, icon: Icon }) => {
            ...
            return (
              ...
              >
                ...
                <span className="whitespace-nowrap text-sm font-medium md:opacity-0 md:transition-opacity md:duration-200 md:group-hover/sidebar:opacity-100">{label}</span>
              </Link>
            );
          })}
        </nav>
        ...
        <div className="flex flex-col gap-1 px-3 pb-2">
          <div className="flex h-11 items-center gap-3 rounded-xl px-3 text-white/55 transition-colors hover:bg-white/10 hover:text-white">
            ...
            <span className="whitespace-nowrap text-sm font-medium md:opacity-0 md:transition-opacity md:duration-200 md:group-hover/sidebar:opacity-100">Sign out</span>
          </div>
        </div>
      </>
    );
    ...
    <div className="flex min-h-screen bg-cn-canvas font-sans text-cn-ink">
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-40 md:hidden" onClick={() => setMobileOpen(false)}>
          ...
          <aside className="absolute bottom-0 left-0 top-0 flex w-64 flex-col bg-cn-sidebar py-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
      ...
      <div className="flex min-h-screen flex-1 flex-col md:pl-[5.25rem]">
        <header className="sticky top-0 z-20 border-b border-cn-border bg-cn-canvas/95 px-4 py-3 backdrop-blur-md sm:px-8">
          <div className="mx-auto flex max-w-6xl items-center gap-4">
            {/* Mobile hamburger */}
            <button type="button" className="flex h-11 w-11 items-center justify-center rounded-xl border border-cn-border bg-cn-surface text-cn-ink-muted transition hover:text-cn-ink md:hidden" onClick={() => setMobileOpen(true)} aria-label="Open menu">
            ...
            <div className="ml-auto flex items-center gap-3">
              {monthlyEarnings !== undefined && (
                <div className="hidden items-center gap-1.5 rounded-full bg-emerald-500/10 px-3 py-1.5 sm:flex">
                  <span className="text-xs font-bold text-emerald-600 dark:text-emerald-400">${monthlyEarnings.toFixed(2)}</span>
              ...
              <Link href="/coach/settings" className="flex items-center gap-2.5">
                <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-indigo-500/20 text-sm font-bold text-indigo-400 ring-2 ring-cn-surface">{initials}</span>
                <span className="hidden min-w-0 sm:block">
                  <span className="flex items-center gap-1.5">
                    <span className="block truncate text-sm font-bold text-cn-ink">{displayName}</span>
                    {isVerified ? <Badge variant="success" size="sm" dot>Verified</Badge> : <Badge variant="warning" size="sm" dot>Pending</Badge>}
                  </span>
